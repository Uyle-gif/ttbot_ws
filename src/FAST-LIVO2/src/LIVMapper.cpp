...
void LIVMapper::publish_frame_world(const rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr &pubLaserCloudFullRes, VIOManagerPtr vio_manager)
{
  ...
  laser360msg.header.frame_id = "map";
  ...
  if (img_en && laserCloudWorldRGB->size() > 0)
  {
    ...
    if (pubLaserCloudMap->get_subscription_count() > 0) 
    {
      ...
      laserRGBmsg.header.frame_id = "map";
...
void LIVMapper::publish_odometry(const rclcpp::Publisher<nav_msgs::msg::Odometry>::SharedPtr &pubOdomAftMapped)
{
  ...
  // br->sendTransform(geometry_msgs::msg::TransformStamped(createTransformStamped(transform, odomAftMapped.header.stamp, "map", "odom")));
